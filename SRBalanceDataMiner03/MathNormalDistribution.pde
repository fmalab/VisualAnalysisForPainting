...
double Mean(double [] a) {
  ...
  return tempSum/a.length;
}
// ------------------ Mean Function -------------------

// ------------------ standard deviation Function -----
double StandardDiv(double mean, double [] a) {
  double temp=0;
  for (int i=0; i<a.length; i++) {
    temp=temp+(a[i]-mean)*(a[i]-mean);
  }
  temp = temp/(a.length-1);
  return java.lang.Math.sqrt(temp);
}
// ------------------ standard deviation Function -----

// ------------------ Normal Distribution Function ----
double Normal_Distribution (double x, double mean, double sd) {
  ...
  String s = String.format("%.4f", z);
  z = Double.parseDouble(s); 
  //    println("Normal Distribution:"+z);
  if (tempx>mean) { 
    return z;
  } else {
    return 1-z;
